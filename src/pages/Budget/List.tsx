...
export default function BudgetList() {
  ...
  const handleExcluir = async (u: Budget) => {
    try {
      ...
      showDeletedToast();
      setBudgets((prev) => prev.filter((c) => c.id !== u.id));
    } catch (error) {
      ...
      render: (u: Budget) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-40">
            ...
            <DropdownMenuItem onClick={() => handleEditar(u)}>
              Enviar email
              <DropdownMenuShortcut><MailIcon className="h-4 w-4"/></DropdownMenuShortcut>
            </DropdownMenuItem>
            {(u.status !== "Aprovado" && u.status !== "Rejeitado" && u.status !== "Encerrado") && (
              <DropdownMenuSub>
                <DropdownMenuSubTrigger>Status</DropdownMenuSubTrigger>
                <DropdownMenuSubContent className="w-40">
                  <DropdownMenuItem onClick={() => handleStatus(u, "approve")}>Aprovar</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleStatus(u, "reject")}>Rejeitar</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleStatus(u, "contest")}>Contestar</DropdownMenuItem>
